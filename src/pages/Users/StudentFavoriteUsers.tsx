...
const StudentFavoriteUsers = () => {
    ...
    const getUsers = async () => {
        get(ref(database, "quiz")).then((quiz)=>{
            ...
            get(ref(database, `users`)).then((snapshot)=>{
                ...
                let data = Object.values(snapshot.val()).filter((user: any) => !!user.email && user.role === "professor" && favorites.users.includes(user.id)).map((user: any) => {
                    return {...user, phone: user.phone ? user.phone : "Unknown"}
                })
                let dataSnapshot = data.map((user: any) => {
                    return {...user, phone: user.phone ? user.phone : "", created: quizes.filter((quiz: any) => quiz.author === user.id).length}
    ...
    return(
        <>
            <AppTopMenu menuLinks={studentsUsersmenuLinks} current="favorites" type="users" />
